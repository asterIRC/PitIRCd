...
#define is_chmode_q(x)  ((x) && (x)->flags & CHFL_OWNER) /* does not check if owner is enabled, should typically not be used */
...
#define IsChannelName(name) ((name) && ((name)[0] == '#' || (name)[0] == '&' || (name)[0] == '!' || (name)[0] == '+' || (name)[0] == '.'))
